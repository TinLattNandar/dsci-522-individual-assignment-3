...
We present the standard deviations in @tbl-standard-deviation.

```{python}
#| echo: false
#| label: tbl-standard-deviation
...
Note that we define standard deviation (of a sample) (@eq-standard-deviation) as

$$s = \sqrt{\frac{\sum_{i=1}^N (x_i - \overline{x})^2}{N-1} }$$ {#eq-standard-deviation}

Additionally, note that in @tbl-standard-deviation we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pop.

{{< pagebreak >}}

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse-dev width=60%}
